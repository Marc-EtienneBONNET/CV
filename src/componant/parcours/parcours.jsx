...
function PageParcours()
{
    let events = [{lien:'https://www.lhsl.fr/', theme:['Tout','Diplôme','Événement majeur'], deb:'07/2017', titre:'Baccalauréat technologique hôtelier' , icon:'coupe'},
                    ...
                    {lien:'', theme:['Tout','Expérience professionnelle','Diplôme'], deb:'07/2020', titre:'Obtention du concours de l\'école 42 (la piscine)',  icon:'coupe'},
    ...
    function createEvenent(type)
    {
        let tmp = events.map((element) => {
            ...
            if (type === 1)
                css = (element.icon === ''? 'borderB':'borderB2') ;
                ...
                return (
                    <div style={{opacity:(element.theme.indexOf(theme) != -1?100:0)}}>
                        <div className={css}>
                            ...
                            <div className={"eventInfo"} key={element.deb}>
    ...
    function createPickEvenent(type)
    {
        ...
        let tmp = events.map((element) => {
            ...
            {
                return (
                    <div className="event" style={{opacity:(element.theme.indexOf(theme) != -1?100:0)}} key={element.titre}></div>
                );
            }
        })
        return (tmp);
